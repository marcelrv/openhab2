...
public class MaxCubeDevicesDiscover  extends AbstractDiscoveryService implements DeviceStatusListener {
	...
	public void onDeviceAdded(MaxCubeBridge bridge, Device device) {
		...
		case HeatingThermostatPlus:
			thingUID = new ThingUID(MaxCubeBinding.HEATHINGTHERMOSTAT_THING_TYPE,MaxCubeBinding.DEVICE_THERMOSTAT + "_" +  device.getSerialNumber());
			break;
		case ShutterContact:
		case EcoSwitch:
			thingUID = new ThingUID(MaxCubeBinding.SWITCH_THING_TYPE, device.getType() + "_" +  device.getSerialNumber());
			break;
		default:
			break;
		}
